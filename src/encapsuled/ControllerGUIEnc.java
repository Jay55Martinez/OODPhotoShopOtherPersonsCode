...
public class ControllerGUIEnc implements ImageController, ActionListener {
   ...
   * reassignment.
   */
  private final ImageViewEncInt view;
   ...
   */
  public ControllerGUIEnc(ImageModelMosaic model, ImageViewEncInt view)
    ...
    commands.put("mosaic", (Scanner s) -> new Mosaic(s.next(), s.next(), s.nextInt()));
  }
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    switch (command) {
      ...
      case "display":
        ...
        break;
      case "mosaic":
        this.actionProcessor2(command, this.view.getNewTitle(), this.view.getDisplayedImage());
        break;
      default:
   ...
   * @param displayedImage - the current displayed image
   */
  private void actionProcessor2(String command, String savedImage, String displayedImage) {
    if (savedImage.equals("")) {
      this.view.showErrorMessage("Give your modified image a name");
    } else {
      try {
        System.out.println(command + " " + displayedImage + " " + savedImage + " " +
                this.view.getMosaicCommand());
        processCommand(command + " " + displayedImage + " " + savedImage + " " +
                this.view.getMosaicCommand());
      } catch (Exception ex) {
        view.showErrorMessage(ex.getMessage()); // show error message if exception caught
      }
      this.view.setPhoto(savedImage); // set the photo in view to the savedImage which will make
      // the displayed image whatever was just created.
    }
  }
}
